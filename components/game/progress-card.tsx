...
export function ProgressCard({ progress }: { progress: UserProgress }) {
  ...
  return (
    <Card className="overflow-hidden border-orange-200/80 bg-gradient-to-br from-orange-50 to-white">
      <CardContent className="space-y-4 p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-orange-700">Nivel {bar.level}</p>
            <p className="text-2xl font-black tabular-nums">{progress.xp} XP</p>
            {title ? <p className="text-sm font-medium text-orange-800">{title}</p> : null}
          </div>
...
function StatChip({
  ...
  return (
    <div className="min-w-[4.25rem] rounded-2xl bg-white/80 px-2.5 py-2 text-center shadow-sm">
